...
from mmaction2.demo.mmaction.apis1 import init_recognizer
from mmaction2.demo.mmaction.datasets1.pipelines import Compose
...
class Inference(QThread):  # 线程类
    ...
    def parse_args(self):
        ...
        parser.add_argument('--config',
                            default='mmaction2/configs/recognition/tanet/tanet_r50_dense_1x1x8_100e_kinetics400_rgb.py',
                            help='test config file path')
        # 本地配置文件的地址
        parser.add_argument('--checkpoint',
                            default='mmaction2/checkpoints/tanet_r50_dense_1x1x8_100e_kinetics400_rgb_20210219-032c8e94.pth',
        ...
        return args
    def parse_args2(self):
        parser = argparse.ArgumentParser(description='MMAction2 webcam demo')
        # 创建配置文件
        parser.add_argument('--config',
                            default='mmaction2/configs/recognition/tsn/tsn_r50_video_inference_1x1x3_100e_kinetics400_rgb.py',
                            help='test config file path')
        # 本地配置文件的地址
        parser.add_argument('--checkpoint',
    ...
    def parse_args3(self):
        ...
        # 本地配置文件的地址
        parser.add_argument('--checkpoint',
                            default='mmaction2/checkpoints/timesformer_divST_8x32x1_15e_kinetics400_rgb-3f8e5d03.pth',
                            help='checkpoint file')
        # 权重文件
        parser.add_argument('--label', default='mmaction2/tools/data/kinetics/label_map_k400.txt', help='label file')
        # 标签文件
...
class ShowResults(QThread):
    ...
    def run(self):
        ...
        while True:  # 循环运行
            ...
            # 上面if做完之后显示画面
            convert_to_qt_format = QImage(frame.data, frame.shape[1], frame.shape[0],
                                          QImage.Format_BGR888)  # 转成qt格式！！！！！
            # print(type(convert_to_qt_format))
            self.update_image.emit(convert_to_qt_format)
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.show_results = ShowResults()  # 初始化（实例化）两个线程

        self.show_results.update_date.connect(self.handleDisplay)
        self.show_results.update_image.connect(self.imageDisplay)

        self.ui.showlabel.setScaledContents(True)
        self.ui.labeltext.setStyleSheet('''*{
    background:#f5f5f5;
    font-size:18px;
    color:#7A7A7A;
    ...
    text-align: center;
        ...
        self.ui.cb.currentIndexChanged[str].connect(self.modechang2)
        self.ui.left_button_1.clicked.connect(self.modechange1)

        self.thread = None  # 创建线程

    def modechang2(self,i):
        self.ui.labelshow.setText('您当前使用的是：动作识别模块  '+i+'模型')
        if i=='TaNet':
            self.react()
        elif i=='TSN':
            self.react2()
    ...
    def handleDisplay(self, data):
        self.ui.labeltext.setText(data)

    def imageDisplay(self, data):
        self.ui.showlabel.setPixmap(QPixmap.fromImage(data))


    def modechange1(self):
        self.ui.labelshow.setText('您当前使用的是：骨架生成模块  HRNet模型')
        self.thread = threading.Thread(target=self.detect)  # 线程调用的是detect
        self.thread.start()
        print("线程开启")

    def activity_show(self):  # 创建线程不断调用摄像头捕捉画面

        while True:
            event.wait()  # 同步问题，生产消费者问题，阻塞等
            print('111')
            print('re')
            if len(result_queue) != 0:
                # 结果队列如果不等于0，结果帧有东西
    ...
    def detect(self):
        ...
        while self.cap.isOpened():
            ...
            self.ui.showlabel.setPixmap(QPixmap.fromImage(convert_to_qt_format))  # 显示
        # self.cap.release()
        cv2.destroyAllWindows()
    ...
    def react(self):
        global frame_queue, camera, frame, results, threshold, sample_length, \
            ...
            result_queue, drawing_fps, inference_fps, event

        event = Event()

        args = self.inference.parse_args()
    ...
    def react2(self):
        ...
        args = self.inference.parse_args2()
    ...
    def react3(self):
        ...
        args = self.inference.parse_args3()
