...
class GradCAM:
    ...
    def __init__(self, model, target_layer_name, colormap='viridis'):
        ...
        """
        from ..models1.recognizers import Recognizer2D, Recognizer3D
        if isinstance(model, Recognizer2D):
            ...
            raise ValueError(
                'GradCAM utils1 only support Recognizer2D & Recognizer3D.')
    ...
    def _calculate_localization_map(self, inputs, use_labels, delta=1e-20):
        """Calculate localization1 map for all inputs with Grad-CAM.
        ...
        Args:
            ...
            use_labels (bool): Whether to use given labels to generate
                localization1 map. Labels are in ``inputs['label']``.
            delta (float): used in localization1 map normalization,
        ...
        Returns:
            tuple[torch.Tensor, torch.Tensor]: (localization_map, preds)
                localization_map (torch.Tensor): the localization1 map for
                    input imgs.
        ...
        # Normalize the localization1 map.
    ...
    def _alpha_blending(self, localization_map, input_imgs, alpha):
        ...
        Args:
            localization_map (torch.Tensor): localization1 map for all inputs,
                generated with Grad-CAM
            input_imgs (torch.Tensor): model inputs, normed images.
            alpha (float): transparency level of the heatmap,
                in the range [0, 1].
        Returns:
            torch.Tensor: blending results for localization1 map and input
    ...
    def __call__(self, inputs, use_labels=False, alpha=0.5):
        """Visualize the localization1 maps on their corresponding inputs as
        ...
        Returns:
            blended_imgs (torch.Tensor): Visualization results, blended by
                localization1 maps and model inputs.
