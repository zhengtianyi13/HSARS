...
def match_by_tag(inp, params):
    ...
    tag_k, loc_k, val_k = inp   #标签，位置，值
    default_ = np.zeros((params.num_joints, 3 + tag_k.shape[2]))  #创建一个0矩阵，形状为关节数，3+tag的什么
    ...
    for i in range(params.num_joints):
        ...
        tags = tag_k[idx]
        joints = np.concatenate(  #关节s 等于位置，值，tag，1
...
class Params(object):
    def __init__(self, cfg):
        self.num_joints = cfg.DATASET.NUM_JOINTS  #关节数
        self.max_num_people = cfg.DATASET.MAX_NUM_PEOPLE  #最大的人数

        self.detection_threshold = cfg.TEST.DETECTION_THRESHOLD  #检测阈值  0.2
        self.tag_threshold = cfg.TEST.TAG_THRESHOLD  #标签阈值  默认1
        self.use_detection_val = cfg.TEST.USE_DETECTION_VAL   #检测值，我也不知道什么东西
        self.ignore_too_much = cfg.TEST.IGNORE_TOO_MUCH  #忽略什么东西啊  是bool值

        if cfg.DATASET.WITH_CENTER and cfg.TEST.IGNORE_CENTER:  #关节顺序
            self.num_joints -= 1
...
class HeatmapParser(object):  #热图解析器的类
    def __init__(self, cfg):
        self.params = Params(cfg)   #传入参数
        self.tag_per_joint = cfg.MODEL.TAG_PER_JOINT  #每个关节点的tag，是true和false
        self.pool = torch.nn.MaxPool2d(
            cfg.TEST.NMS_KERNEL, 1, cfg.TEST.NMS_PADDING  #非极大抑制的核（默认为3），1（未知参数应该是步长），padding，这里是创建一个maxpool
        )

    def nms(self, det):  #非极大抑制
        maxm = self.pool(det)
        maxm = torch.eq(maxm, det).float()
        det = det * maxm
        return det

    def match(self, tag_k, loc_k, val_k):    #匹配  什么tag  位置在哪  值多少（概率？）
        match = lambda x: match_by_tag(x, self.params)  #通过tag match（也是一个函数）
        return list(map(match, zip(tag_k, loc_k, val_k)))  #返回一个列表，列表里面是字典

    def top_k(self, det, tag):  #最好的k个
        # det = torch.Tensor(det, requires_grad=False)
        # tag = torch.Tensor(tag, requires_grad=False)

        det = self.nms(det)  #极大抑制
        num_images = det.size(0)  #图片数
        num_joints = det.size(1)  #关节数
        h = det.size(2)           #高
        w = det.size(3)           #宽
        det = det.view(num_images, num_joints, -1)  #变换矩阵形状
    ...
    def adjust(self, ans, det):   #关节位置调整？？
    ...
    def refine(self, det, tag, keypoints):  #给一个初始化的预测，我们指出丢失的关节
        """
    ...
    def parse(self, det, tag, adjust=True, refine=True):  #应该是解析的主函数了
        ans = self.match(**self.top_k(det, tag))

        if adjust:
            ans = self.adjust(ans, det)  #ans应该是（batchid，peopleid，jointid，x，y）顺序可能不一样，但这也不一定吧
