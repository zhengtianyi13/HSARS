...
def get_multi_stage_outputs(
    ...
    # outputs = []
    heatmaps_avg = 0  #标签平均值
    num_heatmaps = 0  #热图数量
    heatmaps = []     #热图列表
    tags = []         #标签列表

    outputs = model(image)    #把图片送入模型获得预测
    for i, output in enumerate(outputs):
        if len(outputs) > 1 and i != len(outputs) - 1:
            output = torch.nn.functional.interpolate(  #根据给定的size或scale_factor参数来对输入进行下/上采样
                output,
    ...
    if with_flip:
        ...
        heatmaps_avg = 0
        num_heatmaps = 0
        outputs_flip = model(torch.flip(image, [3]))  #反转输出
        for i in range(len(outputs_flip)):  #同样进行上采样
