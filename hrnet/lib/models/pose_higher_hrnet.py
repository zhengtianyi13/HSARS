...
class PoseHigherResolutionNet(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.bn2(x)
        x = self.relu(x)
        #模型经过stem，两个strided 3x3conv进行快速降维
        x = self.layer1(x)
        # 构建了一串BasicBlock构成的模块，阶段1

        # 然后是多个stage，每个stage核心是调用HighResolutionModule模块
        x_list = []  #新的xlist，记录下每个分支
...
#阶段4
        ...
        final_outputs = []  #最后输出
        x = y_list[0]   #第一个高分辨率层的输出
        y = self.final_layers[0](x)   #第一个高分辨率的层
        final_outputs.append(y)   #放入特征

        for i in range(self.num_deconvs):  #进行几次反卷积
            if self.deconv_config.CAT_OUTPUT[i]:
                x = torch.cat((x, y), 1)  #合并操作吧

            x = self.deconv_layers[i](x)  #反卷积层
            y = self.final_layers[i+1](x)
            final_outputs.append(y)

        return final_outputs  #两层？？，一个是原先的输出，一个是更高分辨率的特征
