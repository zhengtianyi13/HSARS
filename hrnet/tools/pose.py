...
from hrnet.lib.config import check_config
from hrnet.lib.core.inference  import get_multi_stage_outputs
from hrnet.lib.core.inference import aggregate_results
...
from hrnet.lib.utils.vis import save_valid_image
from hrnet.lib.utils.transforms import resize_align_multi_scale
from hrnet.lib.utils.transforms import get_final_preds
from hrnet.lib.utils.transforms import get_multi_scale_size
...
#
#     return args
#原来的黑框输入参数的方法，现在参数要写死就不需要了

def main(image):  #主函数

    yamlPath = "D:\\Git\\zty\\HigherHRNet-Human-Pose-Estimation0\\experiments\\coco\\higher_hrnet\\w32_512_adam_lr1e-3.yaml" #改成手动获取参数
    modelpath="D:\\PyCharm\\pose\\hrnet\\models1\\pytorch\\pose_coco\\pose_higher_hrnet_w32_512.pth"  #手动设置模型地址


    # f = open(yamlPath, 'r', encoding='utf-8') #直接用open打开
    # arg = f.read() #读取
    # args = yaml.load(arg,Loader=yaml.BaseLoader)  # 用load方法转字典yaml包中的load方法
    #这个参数暂时没有什么用
    ...
    #代替原来的update_config(cfg, args)，将yaml里的参数更新到默认的cfg里面
